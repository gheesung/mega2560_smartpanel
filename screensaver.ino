...
//
// copy all your BMP files to the s directory on the microSD with your PC
...
void loopImage()
{
    ...
    File f = rootFolder.openNextFile();
    uint8_t ret;
    uint32_t start;
    if (f != NULL) {
#ifdef USE_SDFAT
    ...
    else rootFolder.rewindDirectory();
}
...
uint8_t showBMP(char *nm, int x, int y)
{
    ...
    if ((x >= myGLCD.width()) || (y >= myGLCD.height()))
        return 1;               // off screen
    ...
    if (bmpID != 0x4D42) ret = 2; // bad ID
        ...
        w = bmpWidth;
        h = bmpHeight;
        if ((x + w) >= myGLCD.width())       // Crop area to be loaded
            w = myGLCD.width() - x;
        if ((y + h) >= myGLCD.height())      //
            h = myGLCD.height() - y;
        ...
        myGLCD.setAddrWindow(0, 0, myGLCD.width() - 1, myGLCD.height() - 1); //restore full screen
        ret = 0;        // good render
    }
    bmpFile.close();
    return (ret);
